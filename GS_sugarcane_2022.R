...
phenotype_sheets <- map(sheet_names, ~ cbind(variable = ., read_xlsx('project/data/Phenotype_updated_2017-18_IL_analysis_120921.xlsx', sheet = ., na = c('.', '-', '-!'))))
...
snp_columns <- setdiff(names(genotypes), id_columns)
...
geno_mat_raw <- as.matrix(genotypes[, ..snp_columns])

# Get ID of first and second allele. Map them to 0 and 2 respectively. Map all other values within the row to 1.
...
phen_trait_columns <- c("stkwt_kg", "diam", "Brix", "Fiber", "Pol", "Sucrose", 
                     "Purity", "stalk_ha", "TCH", "TRS", "CRS", "TSH", "EI", "Diam", 
                     "BR", "OR")

pheno_means <- phenotypes[, lapply(.SD, mean, na.rm = TRUE), by = .(variable, Clone), .SDcols = phen_trait_columns]
