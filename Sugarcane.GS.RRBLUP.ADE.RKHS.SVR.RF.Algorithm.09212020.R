...
diploid.polyploid.5cfv <- function(phenames=NULL, diploid.Geno=NULL, m_train.pheno=NULL, species=NULL, k=NULL, cycles = NULL,
  ...
  for(l in 1:length(phenames)){ #length(phenames)#1:length(phenames)
    ...
    r.gy.AD.diplo <- matrix(NA, cycles, k)
    ...
    for (f in 1:k){
      ...
      # Perform GS for each training size
        ...
        E1.2 <- E.mat(Geno.comb.2,shrink=TRUE)
        
        Za.2 <- diag(length(Pheno.comb.2[,1])) 
        Zd.2 <- diag(length(Pheno.comb.2[,1]))
        Ze.2 <- diag(length(Pheno.comb.2[,1]))
        ...
        r.gy.AD.diplo[r,f] <- cor(predicted.2, ww[,1], use="complete")
        the.fitted.model.AD <- lm(ww[,1] ~ predicted.2)
        the.coefficients.AD <- c(the.fitted.model.AD$coefficients[1], the.fitted.model.AD$coefficients[2])
        AD.Intercept[r,f] <- the.coefficients.AD[1]
        AD.Slope[r,f] <- the.coefficients.AD[2]
        
        these.observed.and.predicted.phenotypic.values.AD.diplo <- data.frame(rownames(pheno_testset), ww[,1], predicted.2)
        rownames(these.observed.and.predicted.phenotypic.values.AD.diplo) <- NULL
        colnames(these.observed.and.predicted.phenotypic.values.AD.diplo) <- c("Taxa", "Observed.Value", "Predicted.Value")
        x.p.AD.diplo=these.observed.and.predicted.phenotypic.values.AD.diplo[,c(1,3)]
        y.o.AD.diplo=these.observed.and.predicted.phenotypic.values.AD.diplo[,c(1,2)]
        write.table(these.observed.and.predicted.phenotypic.values.AD.diplo, paste("these.observed.and.predicted.phenotypic.values.AD.diplo.Trait", phenames[l], "Validation.Set", f,"Cycle", r, "txt",sep="."), sep="\t", quote=F, row.names=F, col.names=T)
        AD.CI[r,f] <- round(CI(x.p.AD.diplo,y.o.AD.diplo,s=0.2,top=T),2)

        
        
        #RKHS
        library(BGLR)
        X.2 <- diploid.Geno[match(rownames(Geno.comb.2), rownames(diploid.Geno)),]
        #X.2 <- Geno.comb.2
        ...
        svm.model <- svm(Geno_trainingset.2.rrblup, Yt, type='eps-regression', kernel='radial', scale=FALSE)
        pred <- predict(svm.model, Geno_testset.2.rrblup, decision.values = TRUE)
        ...
        Yt <- as.numeric(Yt)
        rf <- randomForest(Geno_trainingset.2, Yt, tree=5000, mtry=floor(sqrt(ncol(Geno_trainingset.2.rrblup))), 
        ...
        RF.CI[r,f] <- round(CI(x.p.RF.diplo,y.o.RF.diplo,s=0.2,top=T),2)
          ...
          r.gy.AD.diplo.df <- as.data.frame(r.gy.AD.diplo)
          r.gy.AD.diplo.df$Model <- rep("ADE", nrow(r.gy.AD.diplo.df))
          ...
          Prediction.Accuracy.By.Trait <- rbind(r.gy.diplo.df, r.gy.AD.diplo.df, r.gy.RK.diplo.df, r.gy.SV.diplo.df, r.gy.RF.diplo.df)
          ...
          AD.Intercept.df <- as.data.frame(AD.Intercept)
          AD.Intercept.df$Model <- rep("ADE", nrow(AD.Intercept.df))
          ...
          Intercept.By.Trait <- rbind(RB.Intercept.df, AD.Intercept.df, RK.Intercept.df, SV.Intercept.df, RF.Intercept.df)
          ...
          AD.Slope.df <- as.data.frame(AD.Slope)
          AD.Slope.df$Model <- rep("ADE", nrow(AD.Slope.df))
          ...
          Slope.By.Trait <- rbind(RB.Slope.df, AD.Slope.df, RK.Slope.df, SV.Slope.df, RF.Slope.df)
          ...
          AD.CI.df <- as.data.frame(AD.CI)
          AD.CI.df$Model <- rep("ADE", nrow(AD.CI.df))
          ...
          CI.By.Trait <- rbind(RB.CI.df, AD.CI.df, RK.CI.df, SV.CI.df, RF.CI.df)
